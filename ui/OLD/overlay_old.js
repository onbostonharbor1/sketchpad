/*  overlay.js
    Handles the floating overlay with title and control buttons (Prev / Next / Save)
    Used in Patterns and Gallery modes.
*/
...
function injectOverlayIfNeeded() {
  ...
  // ============================================
  // ✅ Determine correct parent container
  //    DRAW mode → active draw subtab pane
  //    PATTERNS mode → #viewer (original behavior)
  // ============================================
  let parent = null;

  if (gl.currentMode === "draw") {
    // Find the active draw-object pane (where canvas lives)
    parent = document.querySelector('#drawSubtabContent .tab-pane.active');
  } else {
    // Fallback to old behavior for patterns/gallery
    parent = document.getElementById("viewer");
  }

  if (!parent) return;

  // Ensure the parent can host absolutely positioned overlay
...
function handleNext() {
  ...
  const currentIndex = wrappers.findIndex((w) =>
...
function handleSave() {
  ...
  const bounds = getDrawnBoundsFrom(snapshotCtx, snapshotCanvas, "#ffffff");
  if (!bounds) {
  ...
  // Thumbnail generation (36 px wide)
...
function getDrawnBoundsFrom(ctx, canvas, bg = "#ffffff", tolerance = 8) {
