/* ===========================================================
   ...
   -----------------------------------------------------------
   Utilities Lab script contract (for parameterControls):
     - exports scriptInfo
   ...
   • Alpha slider (single control, applied to both ends)
   • Steps slider
...
/* ===========================================================
   runPattern()
   -----------------------------------------------------------
   Uses global ctx (your rule).
   Utilities will build the controls by calling:
     buildParameterControls(mod.scriptInfo, "...", ...)
=========================================================== */
export function runPattern() {

  // Alias for parameterControls (drawRegistry vocabulary)
  scriptInfo.params = scriptInfo.parameters;

  // Ensure redrawHandler exists before Utilities wires control events.
  scriptInfo.redrawHandler = function () {
    drawRamp(scriptInfo.parameters);
  };
...
function drawRamp(params) {
  ...
  if (!canvas) throw new Error("drawRamp: #sharedCanvas missing");
  ...
  for (let i = 0; i < colors.length; i++) {
    ...
    // label periodically (0, last, and every 4)
