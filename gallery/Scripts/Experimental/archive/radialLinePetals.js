...
function rgbaToRgb(rgbaText) {
  ...
  const nums = String(rgbaText).match(/[\d.]+/g);
  if (!nums || nums.length < 3) {
    throw new Error("rgbaToRgb: cannot parse: " + String(rgbaText));
  }
...
function update(params) {
  ...
  e.startColor = params.startColor;
  e.endColor   = params.endColor;
  e.space      = params.space;
...
=========================================================== */
function draw() {
  drawRamp(scriptInfo.elements.element);
} // end draw
...
export const scriptInfo = {
  ...
  controls: {
    startColor: { label: "Start", widget: "colorPicker", default: "#0000ff" },
    endColor:   { label: "End",   widget: "colorPicker", default: "#00ff00" },

    space:      { label: "Space", widget: "select", options: ["rgb", "hsl"], default: "rgb" },

    alpha:      { label: "Alpha", widget: "range", min: 0.0, max: 1.0, step: 0.01, default: 1.0 },
    steps:      { label: "Steps", widget: "range", min: 2,   max: 64,  step: 1,    default: 16 }
  }, // end controls
...
export function runPattern() {
  ...
  // compatibility shim for parameterControls
