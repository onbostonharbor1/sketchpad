...
export const scriptInfo = {
  ...
  draw() {
    // Note: ensure 'ctx' is available in your global scope or passed in
    ctx.save();
  ...
  reseed(params, pts) {
    ...
    // FIX: Update existing objects instead of replacing the array
    if (params.points.length === 0) {
      // Initialize the array only once
      params.points = sourcePoints.map(p => ({ x: p.x, y: p.y }));
    } else {
      // Update the x/y of the existing objects so the interactor reference stays valid
