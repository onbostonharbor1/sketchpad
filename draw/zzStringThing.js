...
/////////////////////////////////////////////////////////////////
    function drawInEllipseOrig(thing) {
	...
	function drawWithin(nodes, skip, color, withinCirc,
		...
		numNodes = length;
	    numNodes--;
	    // skip is deducted from numNodes;
	    numNodes -= skip;
	    // however, if there is a taper, we really need to
	    // substract 2*skip
	    if (withinCirc == END_TAPER || withinCirc == TAPER) {
		numNodes -= skip;
	    }
	    for (; i < numNodes; i++) {
		j = i+skip;
...
// START AND END WITHIN, NO TAPER
	if (thing.withinCirc == START_END) {
	    ...
	    // this is to keep the code clear as to what is
	    // occurring
	    if (thing.endSkip > 0)
		...
		drawLines(nodes,0,LENGTH,color);
	...
	// NOW IN TAPER; THE HARD PART
	             // give some small distance from start of
...
//	    start = drawStart(nodes, skip, color, start, 0);
	    start = drawStart(nodes, skip, color, 0);
	else
//	    start = drawStart(nodes, skip, color, start, DISTANCE);
...
//	  ctx.fillText(text,thing.arm1[0].x, thing.arm1[0].y);
//          text = `(${thing.arm2[1].x.toFixed()}, ${thing.arm2[1].y.toFixed()})`;
//	  ctx.fillText(text,thing.arm2[1].x, thing.arm2[1].y);
//	  ctx.fillStyle=color;
//      }
...
//	printTitle("showColors--Tints and shades: " + col);
	...
	for (let i=0; i<8; i++) {
	    ...
	    tintAmount += .1;
	    ctx.fillStyle = newColor.tint.hex;
	...
	tintAmount = .08;
  	...
  	for (let i=0; i<8; i++) {
	    newColor    = calculateTintAndShade(color,tintAmount);
	    tintAmount += .04;
	    ctx.fillStyle = newColor.shade.hex;
...
//	  drawRegularPolygon(thing);
	  ...
	  ctx.fillText("Draw Regular Polygon",10,30);
	  ctx.fillText("Draw Regular Polygon Corner",10,40);
	  ctx.fillText("Draw Regular Polygon Touch", 10,50);
	  ctx.fillText("  lineWidth",       10,70);
	  ctx.fillText("  midpoint",        10,80);
	  ctx.fillText("  numSteps",        10,90);
	  ctx.fillText("  radius",          10,110);
	  ctx.fillText("  rotate",          10,120);
	  ctx.fillText("  yIncrement: best with just parabolas",10,140);
	  ctx.fillText("  xScale",        10,150);
	  ctx.fillText("  yScale",        10,160);
      }
	...
	printTitle("Test 12b: Circular Parabola");
	  ...
	  tintAmount = .08;
  	  ...
  	  for (let i=0; i<8; i++) {
	      ...
	      tintAmount += .04;
	      ctx.fillStyle = newColor.shade.hex;
	      ctx.fillRect(240,startY,200,80);
	      startY += 80;
	  }	      }
...
//	showColors("blue",500);
        ...
        color = "#6b6bff";
	ctx.fillStyle = color;
	ctx.fillRect(10,200,400,400);

	color = "#9e9eff"
	ctx.fillStyle = toColor(color,40);
	ctx.fillRect(60,100,75,600);

	ctx.fillStyle = toColor(color,60);
	ctx.fillRect(135,100,75,600);
